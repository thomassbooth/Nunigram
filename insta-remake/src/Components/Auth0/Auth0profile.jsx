    ...
    isAuthenticated && (
      <div className = 'flex items-center'>
        <div className = 'flex items-center w-[250px]'>
          ...
          <div className = 'ml-4'>
            <Link to={user.nickname} className = 'text-gray-800 dark:text-white font-medium text-sm'>{user.nickname.toLowerCase()}</Link>
            <p className = 'text-[13px] opacity-50 tracking-tight '>{user.name}</p>
          </div>
        </div>
        <button onClick = {() => {logout()}} className = 'text-xs font-semibold tracking-tight text-blue-500'>Switch</button>
      </div>
    )
  );
};
